...
for t = 1:Trials
   ...
   %Store Data
    ...
    Over_Optimal = (length(Searcher_String)-1-Opt_steps)
    MOPT(t) = Opt_steps;
    MCTS(t) = Over_Optimal;
...
MAvg = mean(MCTS(isfinite(MCTS))); 
MVar = var(MCTS(isfinite(MCTS))); 
MTimeAvg = Mtime/Trials;

%% Print Results

fprintf('For a grid of N = %d, %d Trials, and %d Loops the results are as follows: \n', N, Trials, L)
fprintf('Average steps over optimal (percent): %6.2f \n', MAvg)
