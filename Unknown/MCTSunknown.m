...
%Target is unknown, uniform distribution 
%Torus
   ...
   t
    ...
    %while (Sx~= Tx) || (Sy ~= Ty)
       ...
       if Nx(Sx+1, mod1(Sy + 2,M)) == 0  
            ...
            sy1 = sy; 
            count =0; 
            [tx1, ty1] = randtarget2(N);; 
                while dist(tx1, ty1, sx1, sy1) > rv%random playout
                    count = count + 1; 
                    if count > Limit
                        fprintf('limit! 1\n')
                        break
       ...
       %Play best move
          ...
          Px = [Px Sx];
          Py = [Py Sy] ;
          Mx(Sx+1, Sy + 1) = Mx(Sx+1, Sy+1) + 1; 
    
    MCTS(t) = length(Px); 
    end
    
    %uppercent = U/length(Px)
    %rightpercent = RT/length(Px)
    %downpercent = DW/length(Px)
    %leftpercent = LT/length(Px)
...
mx = [0:N]; 
       ...
       [X, Y] = meshgrid(mx, my); 
       figure
       tit = sprintf('Target at (%d, %d) %d Steps', Tx, Ty, length(Px))
       surf(X,Y,Z)
       title(tit)
